...
def main() -> int:
    ...
    parser.add_argument("--skip-dependency-download", action="store_true")
    ...
    if not args.skip_dependency_download:
        _run(
            [
                args.python,
                "-m",
    ...
    if not args.skip_model_download:
        _prepare_download_venv(download_venv, wheelhouse, requirements, args.python)
...
def _prepare_download_venv(
    ...
    venv.EnvBuilder(with_pip=True, clear=True).create(download_venv)
    venv_python = _venv_python(download_venv)
...
def _write_readme(payload_dir: Path) -> None:
    (payload_dir / "README_OFFLINE_PAYLOAD.md").write_text(
        textwrap.dedent(
            ...
            This payload contains Python wheels plus local Docling/EasyOCR model assets.
